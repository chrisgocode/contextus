...
export function createAchievementService(deps: {
	...
	async function unlock(
		...
		const definition = definitionMap.get(achievementId);
		if (definition === undefined || !definition.active) return;
		await deps.repo.unlock(userId, achievementId, now);
	}

	async function recordAcceptedGuess(event: AcceptedGuessEvent) {
		if (event.source !== "guess") return;

		const { playerStats, history, isFirstEverAttemptForPuzzle } =
			await deps.repo.recordRealGuess(event);
		await applyColorAchievements(event.userId, event.distance, event.now);
		...
		if (event.won) {
			...
			for (const participantUserId of activeGuessers) {
				...
				if (isNewSolve) {
					await applySolveTotals(participantUserId, event.now);
				}
			}
		}
	...
	async function applyColorAchievements(
		...
		if (bucket === "red") {
			stats.redGuesses += 1;
			await applyCounterRules(userId, "redGuesses", stats.redGuesses, now);
			...
			await applyCounterRules(userId, "greenGuesses", stats.greenGuesses, now);
		}
		await deps.repo.saveStats(userId, stats);
	}

	async function applySolveTotals(userId: Id<"users">, now: number) {
		const stats = await deps.repo.getOrCreateStats(userId);
		stats.uniqueSolves += 1;
		await deps.repo.saveStats(userId, stats);
		await applyCounterRules(userId, "uniqueSolves", stats.uniqueSolves, now);
	}
	...
	async function applyCounterRules(
		...
		for (const rule of evaluateCounterRules(counterId, value)) {
			...
			if (rule.shouldUnlock) {
				await unlock(userId, rule.achievementId, now);
			}
		}
	}
