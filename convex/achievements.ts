...
export async function recordAcceptedGuessForAchievements(
	...
	await service.recordAcceptedGuess(event);
}
...
function createConvexAchievementRepository(
	...
	return {
		...
		async recordRealGuess(event) {
			...
			const isFirstEverAttemptForPuzzle =
				history.firstAttemptAt === undefined;
			...
			const stats = {
				...
				lastDistance: event.distance,
				noBacktrackingSoFar:
					existing.noBacktrackingSoFar && event.distance < existing.lastDistance,
			};
		...
		async markSolvedOnce(userId, gameId, contextoGameId, now) {
			const history = await getOrCreateHistory(ctx, userId, contextoGameId, now);
