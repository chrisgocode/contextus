...
export function getUnlockedAchievementMetadata(id: string) {
  const achievement = getAchievementById(id);
  if (achievement === null) return null;
  const group = getAchievementGroup(achievement.category);
  return {
    achievement,
    group,
    trophy:
      achievement.category === "hidden" && group.unlockedImage
        ? group.unlockedImage
        : group.image,
  };
}
